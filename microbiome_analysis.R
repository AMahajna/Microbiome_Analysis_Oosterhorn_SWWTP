...
example <- fread("input_data/subsystems_result/SRR29355732.reduced", header = FALSE, sep = "\t", fill = TRUE)

# Function to read and preprocess each file
read_and_preprocess_metabolism <- function(file) {
  # Extract the file name without extension
  file_name <- tools::file_path_sans_ext(basename(file))
  ...
  # Ensure columns 3 to 6 are of character type
  df[, 3:6] <- lapply(df[, 3:6], as.character)
...
# Read and preprocess all files
data_list_metabolism <- lapply(file_list_metabolism, read_and_preprocess_metabolism)
...
#Check if transformation was done correctly
#sum of rel_abundance columns if 100
columns_sum_metabolism = colSums(merged_data_metabolism[, -c(1:4)], na.rm = TRUE)
