...
subsystem_acc = specaccum(t(assay(tse_subsystem)), method = 'collector')
...
functional_category_acc = specaccum(t(assay(tse_functional_category)), method = 'collector')
...
description_acc = specaccum(t(assay(tse_description)), method = 'collector')
...
function_acc_curve = ggplot() +
  ...
  # Theme and labels
  theme_minimal() +
  labs(x = "Species Richness", y = "Function Richness")
...
enzyme_acc_curve = ggplot() +
  ...
  # Theme and labels
  theme_minimal() +
  labs(x = "Species Richness", y = "Functional Gene Richness")

#+
#  theme(
#    axis.title.x = element_text(size = 8),  # Decrease the x-axis title font size
#    axis.title.y = element_text(size = 8),  # Decrease the y-axis title font size
#  )
...
legend_data <- data.frame(
  ...
  label = c("Legend","Functional Genes", "SEED Subsystems", "Description", "SEED Subsystems Functional Category")
)
...
legend_plot <- legend_plot +
  ...
  annotation_custom(grob = textGrob("Functional Genes", gp = gpar(col = "darkorange", fontsize = 10)), xmin = -0.65, ymin = 0.5) +
